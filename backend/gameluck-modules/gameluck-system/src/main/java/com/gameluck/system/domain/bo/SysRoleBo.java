...
public class SysRoleBo extends BaseEntity {
     ...
     * 角色名称
     */
    @NotBlank(message = "{system.role.name.required}")
    @Size(min = 0, max = 30, message = "{system.role.name.length}")
    private String roleName;

    /**
     * 角色权限字符串
     */
    @NotBlank(message = "{system.role.key.required}")
    @Size(min = 0, max = 100, message = "{system.role.key.length}")
