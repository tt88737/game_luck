export default {
  ...
  paymentSettlement: {
    title: '支付结算', create: '创建结算批次', detail: '结算详情', number: '结算批次号', provider: '支付渠道', currency: '币种', window: 'UTC 结算时段', gross: '支付总额', refunds: '退款', chargebacks: '拒付', fees: '费用', net: '净结算额', payable: '应付渠道', receivable: '应收渠道', calculate: '计算', close: '关闭结算', events: '条事件', items: '事件明细', evidence: '对账证据', history: '操作历史', eventType: '事件类型', orderNo: '订单号', amount: '金额', feePercent: '支付费率', fixedFee: '支付固定费', chargebackFee: '拒付固定费', remark: '关闭备注', openReconciliation: '打开支付对账工作台', noMutation: '关闭结算只记录财务确认，不会修改支付订单、冲正或钱包余额', permissionDenied: '无权查看支付结算', loadFailed: '支付结算加载失败', detailFailed: '结算详情加载失败', retry: '重试', empty: '暂无结算批次', filteredEmpty: '当前筛选条件下无结算批次', emptyItems: '暂无结算事件', emptyHistory: '暂无操作历史', noEvidence: '暂无对账证据', processing: '结算正在计算，请稍后刷新', failed: '结算计算失败', providerRequired: '请输入支付渠道', currencyRequired: '请输入三位币种代码', windowRequired: '请选择 UTC 结算时段', windowInvalid: '结算时段必须大于 0 且不超过 31 天', remarkRequired: '请填写关闭备注', calculateConfirm: '将按已处理支付事件生成不可变财务快照，是否继续？', calculateAccepted: '结算计算已完成', closed: '结算批次已关闭', status: { CREATED: '已创建', CALCULATING: '计算中', CALCULATED: '已计算', CLOSED: '已关闭', FAILED: '失败' }
  },
  paymentSettlementReport: {
    ...
    latest31: '最近 31 天',
    export: '导出 CSV',
    exportDenied: '当前账号没有结算报表导出权限',
    currencyTotals: '逐币种汇总',
    batches: '个批次',
    batchCount: '批次数',
    eventCount: '事件数',
    payments: '支付',
    ...
    latestClose: '最近关闭时间',
    generatedAt: '生成时间',
    sourceBatches: '来源结算批次',
    settlementNo: '结算批次号',
    periodStart: '时段开始',
    periodEnd: '时段结束',
    permissionDenied: '当前账号没有结算报表查看权限',
    exportInProgress: '正在准备导出',
    loadFailed: '结算报表加载失败',
    batchLoadFailed: '来源结算批次加载失败',
    retry: '重试',
    empty: '暂无已关闭结算报表数据',
    filteredEmpty: '当前筛选条件下无报表数据',
    emptyBatches: '暂无来源结算批次',
    dateInvalid: '请选择不超过 31 天且不晚于今天的有效 UTC 日期范围'
  },
  reportTrends: {
    title: '每日趋势',
