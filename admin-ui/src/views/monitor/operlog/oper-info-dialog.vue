<template>
  <el-dialog v-model="open" :title="tt('操作日志详细')" width="700px" append-to-body close-on-click-modal @closed="info = null">
    <el-descriptions v-if="info" :column="1" border>
      <el-descriptions-item :label="tt('操作状态')">
        <template #default>
          <el-tag v-if="info.status === 0" type="success">{{ tt('正常') }}</el-tag>
          <el-tag v-else-if="info.status === 1" type="danger">{{ tt('失败') }}</el-tag>
        </template>
      </el-descriptions-item>
      <el-descriptions-item :label="tt('登录信息')">
        <template #default> {{ info.operName }} / {{ info.deptName }} / {{ info.operIp }} / {{ info.operLocation }} </template>
      </el-descriptions-item>
      <el-descriptions-item :label="tt('请求信息')">
      ...
      <el-descriptions-item :label="tt('请求参数')">
        <template #default>
      ...
      <el-descriptions-item :label="tt('返回参数')">
        <template #default>
          <div class="max-h-300px overflow-y-auto">
      ...
      <el-descriptions-item :label="tt('消耗时间')">
        <template #default>
          <span> {{ info.costTime }}ms </span>
        </template>
      </el-descriptions-item>
      <el-descriptions-item :label="tt('操作时间')">
        <template #default> {{ proxy.parseTime(info.operTime) }}</template>
      </el-descriptions-item>
      <el-descriptions-item v-if="info.status === 1" :label="tt('异常信息')">
        <template #default>
