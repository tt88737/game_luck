  ...
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="来源类型" prop="sourceType">
              <el-input v-model="queryParams.sourceType" placeholder="请输入来源类型" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['wallet:rule:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" border :data="ruleList">
        ...
        <el-table-column label="来源类型" align="center" prop="sourceType" min-width="130" />
        <el-table-column label="规则名称" align="center" prop="ruleName" min-width="150" />
        <el-table-column label="释放模式" align="center" prop="releaseMode" min-width="130">
          <template #default="scope">{{ releaseModeText(scope.row.releaseMode) }}</template>
        </el-table-column>
        <el-table-column label="需要流水" align="center" width="100">
          <template #default="scope">
            <el-tag :type="scope.row.turnoverRequired === '0' ? 'warning' : 'info'">{{ yesNoText(scope.row.turnoverRequired) }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="默认流水" align="right" prop="defaultRequiredTurnover" width="120" />
        <el-table-column label="提现" align="center" width="80">
          <template #default="scope">
            <el-tag :type="scope.row.withdrawEnabled === '0' ? 'success' : 'info'">{{ capabilityText(scope.row.withdrawEnabled) }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="兑换" align="center" width="80">
          <template #default="scope">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="640px" append-to-body>
      <el-form ref="ruleFormRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="来源类型" prop="sourceType">
          <el-input v-model="form.sourceType" placeholder="例如 GAME_PROFIT / DEPOSIT / PROMOTION" />
        </el-form-item>
        ...
        <el-form-item label="释放模式" prop="releaseMode">
          <el-select v-model="form.releaseMode" placeholder="请选择释放模式">
            <el-option label="立即释放" value="IMMEDIATE" />
            <el-option label="满足流水后释放" value="AFTER_TURNOVER" />
            <el-option label="永不释放" value="NEVER" />
            <el-option label="人工审核" value="MANUAL_REVIEW" />
          </el-select>
        </el-form-item>
        <el-form-item label="需要业务流水">
          <el-switch v-model="form.turnoverRequired" active-value="0" inactive-value="1" active-text="需要" inactive-text="不需要" />
        </el-form-item>
        <el-form-item label="默认流水">
...
const data = reactive<PageData<RuleForm, RuleQuery>>({
  ...
  rules: {
    ...
    releaseMode: [{ required: true, message: '请选择释放模式', trigger: 'change' }]
  }
});

const { queryParams, form, rules } = toRefs(data);

const statusText = (value?: string) => (value === '0' ? '启用' : '停用');
const yesNoText = (value?: string) => (value === '0' ? '需要' : '不需要');
const capabilityText = (value?: string) => (value === '0' ? '具备' : '不具备');

const releaseModeText = (value?: string) => {
  if (value === 'IMMEDIATE') return '立即释放';
  if (value === 'AFTER_TURNOVER') return '满足流水后释放';
  if (value === 'NEVER') return '永不释放';
  if (value === 'MANUAL_REVIEW') return '人工审核';
  return value || '-';
};
