  ...
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item :label="tt('订单号')" prop="depositOrderNo">
              <el-input v-model="queryParams.depositOrderNo" :placeholder="tt('请输入订单号')" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['payment:deposit:add']" type="primary" plain icon="Plus" @click="handleAdd">{{ tt('新增') }}</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" border :data="depositList">
        ...
        <el-table-column :label="tt('支付时间')" align="center" prop="payTime" width="170" />
        ...
        <el-table-column :label="tt('操作')" align="center" width="180" fixed="right">
          <template #default="scope">
            ...
            <el-tooltip v-if="scope.row.status === 'PENDING'" :content="tt('模拟支付成功')" placement="top">
            ...
            <el-tooltip v-if="scope.row.status === 'PENDING'" :content="tt('取消订单')" placement="top">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="520px" append-to-body>
      <el-form ref="depositFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item :label="tt('金额')" prop="amount">
...
const data = reactive<PageData<DepositOrderForm, DepositOrderQuery>>({
  ...
  rules: {
    ...
    amount: [{ required: true, message: tt('金额不能为空'), trigger: 'blur' }]
  }
});
...
const handleSimulate = async (row: DepositOrderVO) => {
  await proxy?.$modal.confirm(tt('确认将该充值订单标记为模拟支付成功并执行钱包入账？'));
  await simulateDepositSuccess(row.id);
  proxy?.$modal.msgSuccess(tt('模拟支付成功'));
  await getList();
};

const handleCancel = async (row: DepositOrderVO) => {
  await proxy?.$modal.confirm(tt('确认取消该充值订单？'));
  await cancelDeposit(row.id);
  proxy?.$modal.msgSuccess(tt('取消成功'));
  await getList();
};
