      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item :label="tt('公告标题')" prop="noticeTitle">
              <el-input v-model="queryParams.noticeTitle" :placeholder="tt('请输入公告标题')" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item :label="tt('操作人员')" prop="createByName">
              <el-input v-model="queryParams.createByName" :placeholder="tt('请输入操作人员')" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item :label="tt('类型')" prop="noticeType">
              <el-select v-model="queryParams.noticeType" :placeholder="tt('公告类型')" clearable>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:notice:add']" type="primary" plain icon="Plus" @click="handleAdd">{{ tt('新增') }}</el-button>
          </el-col>
          <el-col :span="1.5">
      ...
      <el-table v-loading="loading" border :data="noticeList" @selection-change="handleSelectionChange">
        ...
        <el-table-column v-if="false" :label="tt('序号')" align="center" prop="noticeId" width="100" />
        <el-table-column :label="tt('公告标题')" align="center" prop="noticeTitle" :show-overflow-tooltip="true" />
        <el-table-column :label="tt('公告类型')" align="center" prop="noticeType" width="100">
        ...
        <el-table-column :label="tt('创建者')" align="center" prop="createByName" width="100" />
        <el-table-column :label="tt('创建时间')" align="center" prop="createTime" width="100">
          <template #default="scope">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="780px" append-to-body>
      <el-form ref="noticeFormRef" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="12">
            <el-form-item :label="tt('公告标题')" prop="noticeTitle">
              <el-input v-model="form.noticeTitle" :placeholder="tt('请输入公告标题')" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="tt('公告类型')" prop="noticeType">
              <el-select v-model="form.noticeType" :placeholder="tt('请选择')">
...
const data = reactive<PageData<NoticeForm, NoticeQuery>>({
  ...
  rules: {
    noticeTitle: [{ required: true, message: tt('公告标题不能为空'), trigger: 'blur' }],
    noticeType: [{ required: true, message: tt('公告类型不能为空'), trigger: 'change' }]
  }
});
...
const handleDelete = async (row?: NoticeVO) => {
  ...
  await proxy?.$modal.confirm(tt('是否确认删除公告编号为') + '"' + noticeIds + '"' + tt('的数据项？'));
