  ...
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item :label="tt('状态')" prop="status">
              <el-select v-model="queryParams.status" :placeholder="tt('角色状态')" clearable>
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10">
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:role:add']" type="primary" plain icon="Plus" @click="handleAdd()">{{ tt('新增') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:role:edit']" type="success" plain :disabled="single" icon="Edit" @click="handleUpdate()">{{ tt('修改') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:role:remove']" type="danger" plain :disabled="ids.length === 0" @click="handleDelete()">{{ tt('删除') }}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['system:role:export']" type="warning" plain icon="Download" @click="handleExport">{{ tt('导出') }}</el-button>
          </el-col>
      ...
      <el-table ref="roleTableRef" border v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
        ...
        <el-table-column v-if="false" :label="tt('角色编号')" prop="roleId" width="120" />
        <el-table-column :label="tt('角色名称')" prop="roleName" :show-overflow-tooltip="true" width="150" />
        <el-table-column :label="tt('权限字符')" prop="roleKey" :show-overflow-tooltip="true" width="200" />
        <el-table-column :label="tt('显示顺序')" prop="roleSort" width="100" />
        <el-table-column :label="tt('状态')" align="center" width="100">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="roleFormRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item :label="tt('角色名称')" prop="roleName">
          <el-input v-model="form.roleName" :placeholder="tt('请输入角色名称')" />
        </el-form-item>
        <el-form-item prop="roleKey">
          <template #label>
            <span>
              <el-tooltip :content="tt(rolePermissionTooltip)" placement="top">
                <el-icon><question-filled /></el-icon>
              </el-tooltip>
              {{ tt('权限字符') }}</span>
          </template>
          <el-input v-model="form.roleKey" :placeholder="tt('请输入权限字符')" />
        </el-form-item>
        <el-form-item :label="tt('角色顺序')" prop="roleSort">
    ...
    <el-dialog v-model="openDataScope" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="dataScopeRef" :model="form" label-width="80px">
        <el-form-item :label="tt('角色名称')">
          <el-input v-model="form.roleName" :disabled="true" />
        </el-form-item>
        <el-form-item :label="tt('权限字符')">
        ...
        <el-form-item v-show="form.dataScope === '2'" :label="tt('数据权限')">
          <el-checkbox v-model="deptExpand" @change="handleCheckedTreeExpand($event, 'dept')">{{ tt('展开/折叠') }}</el-checkbox>
          <el-checkbox v-model="deptNodeAll" @change="handleCheckedTreeNodeAll($event, 'dept')">{{ tt('全选/全不选') }}</el-checkbox>
          <el-checkbox v-model="form.deptCheckStrictly" @change="handleCheckedTreeConnect($event, 'dept')">{{ tt('父子联动') }}</el-checkbox>
          <el-tree
            ref="deptRef"
            class="tree-border"
            :data="deptOptions"
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitDataScope">{{ tt('确定') }}</el-button>
          <el-button @click="cancelDataScope">{{ tt('取消') }}</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const data = reactive<PageData<RoleForm, RoleQuery>>({
  ...
  rules: {
    roleName: [{ required: true, message: tt('角色名称不能为空'), trigger: 'blur' }],
    roleKey: [{ required: true, message: tt('权限字符不能为空'), trigger: 'blur' }],
    roleSort: [{ required: true, message: tt('角色顺序不能为空'), trigger: 'blur' }]
  }
});
...
const handleDelete = async (row?: RoleVO) => {
  ...
  await proxy?.$modal.confirm(tt('是否确认删除角色编号为') + roleids + tt('数据项目'));
...
const handleStatusChange = async (row: RoleVO) => {
  ...
  try {
    await proxy?.$modal.confirm(tt('确认要') + '"' + text + '""' + row.roleName + '"' + tt('角色吗') + '?');
