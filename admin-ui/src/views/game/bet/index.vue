<template>
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <el-form-item label="注单号" prop="betOrderNo">
              <el-input v-model="queryParams.betOrderNo" placeholder="请输入注单号" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            ...
            <el-form-item label="游戏" prop="gameCode">
              <el-input v-model="queryParams.gameCode" placeholder="游戏编码" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="状态" prop="status">
              <el-select v-model="queryParams.status" placeholder="请选择状态" clearable class="!w-150px">
                <el-option label="待下注" value="PENDING" />
                <el-option label="已扣款" value="BET_SUCCESS" />
                <el-option label="扣款失败" value="BET_FAILED" />
                <el-option label="已结算" value="SETTLED" />
                <el-option label="结算失败" value="SETTLE_FAILED" />
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['game:bet:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" border :data="betList">
        <el-table-column label="注单号" align="center" prop="betOrderNo" min-width="180" show-overflow-tooltip />
        ...
        <el-table-column label="游戏" align="center" prop="gameCode" width="120" show-overflow-tooltip />
        <el-table-column label="局号" align="center" prop="roundNo" min-width="150" show-overflow-tooltip />
        <el-table-column label="下注金额" align="right" prop="betAmount" width="120" />
        <el-table-column label="派彩金额" align="right" prop="payoutAmount" width="120" />
        <el-table-column label="净额" align="right" prop="netAmount" width="120" />
        ...
        <el-table-column label="操作" align="center" width="180" fixed="right">
          <template #default="scope">
            ...
            <el-tooltip v-if="scope.row.status === 'PENDING'" content="模拟下注扣款" placement="top">
              <el-button v-hasPermi="['game:bet:place']" link type="primary" icon="CircleCheck" @click="handlePlace(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip v-if="scope.row.status === 'BET_SUCCESS'" content="模拟结算派彩" placement="top">
              <el-button v-hasPermi="['game:bet:settle']" link type="success" icon="Money" @click="handleSettle(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip v-if="scope.row.status === 'BET_SUCCESS'" content="取消退款" placement="top">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="560px" append-to-body>
      <el-form ref="betFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="游戏编码" prop="gameCode">
          <el-input v-model="form.gameCode" placeholder="默认 SIMULATED" />
        </el-form-item>
        <el-form-item label="局号" prop="roundNo">
          <el-input v-model="form.roundNo" placeholder="为空则自动生成" />
        </el-form-item>
        <el-form-item label="下注金额" prop="betAmount">
          <el-input-number v-model="form.betAmount" :precision="6" :min="0.000001" class="w-full" />
        </el-form-item>
        <el-form-item label="派彩金额" prop="payoutAmount">
    ...
    <el-dialog v-model="detailOpen" title="模拟下注订单详情" width="720px" append-to-body>
      <el-descriptions :column="2" border>
        <el-descriptions-item label="注单号">{{ detail.betOrderNo }}</el-descriptions-item>
        ...
        <el-descriptions-item label="游戏">{{ detail.gameCode }}</el-descriptions-item>
        <el-descriptions-item label="局号">{{ detail.roundNo }}</el-descriptions-item>
        <el-descriptions-item label="下注金额">{{ detail.betAmount }}</el-descriptions-item>
...
const data = reactive<PageData<GameBetOrderForm, GameBetOrderQuery>>({
  ...
  rules: {
    ...
    betAmount: [{ required: true, message: '下注金额不能为空', trigger: 'blur' }],
    payoutAmount: [{ required: true, message: '派彩金额不能为空', trigger: 'blur' }]
  }
});
...
const handlePlace = async (row: GameBetOrderVO) => {
  await proxy?.$modal.confirm('确认对该模拟下注订单执行钱包扣款？');
  await placeGameBet(row.id);
  proxy?.$modal.msgSuccess('下注扣款完成');
  await getList();
};

const handleSettle = async (row: GameBetOrderVO) => {
  await proxy?.$modal.confirm('确认对该模拟下注订单执行结算派彩？');
  await settleGameBet(row.id);
  proxy?.$modal.msgSuccess('结算派彩完成');
  await getList();
};

const handleCancelBet = async (row: GameBetOrderVO) => {
  await proxy?.$modal.confirm('确认取消该模拟下注订单并退回下注金额？');
